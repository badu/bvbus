...
const (
	...
	CreatePointsSQL    = `CREATE TABLE IF NOT EXISTS points (id INTEGER, lat REAL, lng REAL, idx INTEGER NOT NULL, is_stop BOOLEAN DEFAULT 0, from_station_id INTEGER NOT NULL, to_station_id INTEGER NOT NULL, CONSTRAINT fk_points_station_from FOREIGN KEY (from_station_id) REFERENCES stations(id) ON DELETE CASCADE, CONSTRAINT fk_points_station_to FOREIGN KEY (to_station_id) REFERENCES stations(id) ON DELETE CASCADE);`
	CreateDistancesSQL = `CREATE TABLE IF NOT EXISTS distances (from_station_id INTEGER NOT NULL, to_station_id INTEGER NOT NULL, distance REAL, minutes REAL, PRIMARY KEY (from_station_id, to_station_id), CONSTRAINT fk_distances_station_from FOREIGN KEY (from_station_id) REFERENCES stations(id) ON DELETE CASCADE, CONSTRAINT fk_distances_station_to FOREIGN KEY (to_station_id) REFERENCES stations(id) ON DELETE CASCADE);`
...
func (r *Repository) CleanupStreetPoints() error {
	_, err := r.DB.Exec(CleanupPointsSQL)
	return err
}
